...
- Timestamp: 2026-08-30 19:20
  ...
  - Scope: `docs/adr/0029-one-operator-surface.md` (amended), `backend/src/awf/{cli/main.py,protocol/methods.py,ops/control.py,server/stdio.py,secrets/cli.py,engine/recovery.py,speech/cli.py}`, `backend/tests/integration/test_cli_main.py`, `scripts/generate_protocol.py`, `frontend/shared/src/{transport.ts,client.ts}`, `frontend/cli/{src/commands.ts,tests/commands.test.ts}`, `frontend/gui/src/main/voicePipeline.ts`, `docs/{OperatorsGuide.md,QuickStart-linux.md,QuickStart-windows.md}`, `CHANGE_LOG.md`.
  - Validation: `backend/.venv/bin/python -m pytest backend/tests` passed (`691 passed, 3 skipped`); `backend/.venv/bin/python scripts/validate_backend.py lint` passed; `scripts/generate_protocol.py --check` and `--check-argparse` passed; `npm --prefix frontend test --workspaces --if-present` passed (`shared 15`, `cli 52`, `gui 74`); `npm --prefix frontend run build` passed; `git diff --check` passed; all fifteen retired spellings were confirmed to exit non-zero and every replacement was confirmed to exit zero against the live database; `awf system serve --stdio` answered an `awf/run.list` request over stdio.
  - Notes: This is a breaking change with no compatibility path. `awf serve --stdio` moved to `awf system serve --stdio`, so `frontend/shared/src/transport.ts` was updated to spawn the new path and a frontend built before this change cannot start a backend after it. Phase 10 and Phase 11 acceptance must be re-derived against ADR-0029's command table; the underlying operations are unchanged but the commands that invoke them are not. JSON-RPC method names are unchanged.

- Timestamp: 2026-08-30 19:08
  - Host class(es): Linux/WSL2 backend and frontend validation
  - Summary: Consolidated the operator surface onto one set of task-shaped concepts under ADR-0029. `awf review` now resolves any decision id - approval, proposed code change, or drafted registry object - and acts on whichever it names; `awf memory` absorbs session and episodic access; the TUI mirrors both groups; and the GUI presents Operate, Chat, and Library instead of seven views.
  - Scope: `docs/adr/0029-one-operator-surface.md` (new), `backend/src/awf/{cli/main.py,protocol/methods.py,ops/{control.py,run.py},improvement/summary.py}`, `backend/tests/integration/test_cli_main.py`, `backend/tests/unit/{test_cli_improvement.py,test_improvement_summary.py}`, `scripts/generate_protocol.py`, `frontend/cli/{src/commands.ts,tests/commands.test.ts}`, `frontend/gui/{src/renderer/App.tsx,tests/{App.nav.test.tsx,App.dashboard.test.tsx}}`, `docs/{OperatorsGuide.md,QuickStart-linux.md,QuickStart-windows.md,adr/0025-control-center-look-usability.md}`, `CHANGE_LOG.md`.
  - Validation: `backend/.venv/bin/python -m pytest backend/tests` passed (`691 passed, 3 skipped`, two new CLI contract tests); `backend/.venv/bin/python scripts/validate_backend.py lint` passed; `scripts/generate_protocol.py --check` and `--check-argparse` passed, with the parity check extended to cover the unlisted spellings; `npm --prefix frontend test --workspaces --if-present` passed (`shared 15`, `cli 52`, `gui 74`); `npm --prefix frontend run build` passed; `git diff --check` passed; each Section 16.1 command was run directly and returned zero, and `awf control`, `awf review list`, and `awf review show <id>` were smoke-tested against the live database.
  - Notes: Listed CLI top-level commands went 22 to 14 and listed TUI slash commands 51 to 30; GUI views went 7 to 3. Spec Sections 16.1 and 16.2 pin command names that Phase 10/11 acceptance runs, so `awf approvals`, `awf approve`, and `awf reject` remain dispatchable as unlisted spellings, every other Section 16.1 command is unchanged, and `/memory <ref>` still resolves directly alongside `/memory get <ref>`. JSON-RPC method names are unchanged; only `cli_path` metadata moved.

- Timestamp: 2026-08-30 18:52
  - Host class(es): Linux/WSL2 backend and frontend validation
  - Summary: Repaired the operator-facing failure and discovery paths across the CLI, TUI, and GUI: every command now renders help and reports errors as messages, unknown workflow refs and schema violations read as instructions, the runs list and readiness summary are scannable, and the TUI help and GUI doctor action lead somewhere.
  - Scope: `backend/src/awf/{cli/main.py,ops/run_execution.py,workflow/io_schema.py}`, `backend/tests/integration/test_cli_main.py`, `frontend/cli/src/commands.ts`, `frontend/cli/tests/commands.test.ts`, `frontend/gui/src/renderer/App.tsx`, `docs/{OperatorsGuide.md,QuickStart-linux.md,QuickStart-windows.md}`, `CHANGE_LOG.md`.
  - Validation: `backend/.venv/bin/python -m pytest backend/tests` passed (`689 passed, 3 skipped`, three new CLI regression tests); the new help-rendering test was confirmed to fail against the pre-fix parser and pass after; `backend/.venv/bin/python scripts/validate_backend.py lint` passed; `scripts/generate_protocol.py --check` and `--check-argparse` passed; `npm --prefix frontend test --workspaces --if-present` passed (`shared 15`, `cli 48`, `gui 74`); `npm --prefix frontend run build` passed; `git diff --check` passed; functional CLI smoke covered `awf --help`, `awf run --help`, `awf run nope@1.0.0`, `awf run assistant-default`, `awf runs`, and `awf readiness`.
  - Notes: Fixed defects were `awf run --help` aborting with `ValueError: empty group` because each mutually exclusive argument created a fresh group; the CLI catching only `CoreOpError` out of 56 backend error classes, so most failures printed a traceback; and the GUI `doctor.open` work-item action re-selecting the current view instead of refreshing. The CLI command surface itself is unchanged - the 22 top-level commands and their overlapping concepts remain as they were.

- Timestamp: 2026-08-30 18:39
  - Host class(es): Linux/WSL2 backend and frontend validation
  - Summary: Closed out the end-to-end operability work stream by bringing the touched backend files to repository formatting standard and promoting the ONNX Whisper availability probe to the public `awf.speech.models` surface it is now called through.
  - Scope: `backend/src/awf/{cli/main.py,ops/control.py,speech/models.py,speech/stt_onnx.py}`, `backend/tests/unit/{test_control_operability.py,test_speech_cli_transcribe.py}`, `backend/tests/runtime/test_voice_pipeline.py`, `CHANGE_LOG.md`.
  - Validation: `backend/.venv/bin/python scripts/validate_backend.py lint` passed (`242 files already formatted`, `All checks passed!`); `backend/.venv/bin/python -m pytest backend/tests` passed (`686 passed, 3 skipped`); `scripts/generate_protocol.py --check` and `--check-argparse` passed; `npm --prefix frontend test --workspaces --if-present` passed (`shared 15`, `cli 48`, `gui 74`); `npm --prefix frontend run build` passed; `git diff --check` passed; functional smoke of `awf control` and `awf status <run-id>` rendered the operator work queue, start options, next actions, and run timeline.
  - Notes: The 2026-08-30 18:31 entry recorded Ruff as passing for the touched files; repository-wide `ruff format --check` still failed on four of them, and this entry records that corrective work. The three runtime skips are absent local STT model artifacts, not evidence of live STT execution.

- Timestamp: 2026-08-30 18:31
  - Host class(es): Linux/WSL2 backend and frontend validation
  - Summary: Completed the operator usability pass on top of the control-center overhaul. Operate now exposes a schema-driven Start work panel, lane-based actionable work queue, typed backend action metadata, registry workflow Run handoff, and run-detail rejection reason capture so operators can start, monitor, unblock, review, and close out work without translating raw commands or hopping between pages.
  - Scope: `backend/src/awf/{ops/control.py,cli/main.py}`, `backend/tests/unit/test_control_operability.py`, `frontend/shared/src/types.ts`, `frontend/cli/src/commands.ts`, `frontend/gui/src/renderer/{App.tsx,Dashboard.tsx,OperatorWorkQueue.tsx,RegistryActions.tsx,RegistryObjectSummary.tsx,RunTimeline.tsx,StartWorkPanel.tsx,styles.css}`, `frontend/gui/tests/App.dashboard.test.tsx`, `docs/{OperatorsGuide.md,QuickStart-linux.md,QuickStart-windows.md}`, `docs/adr/{0025-control-center-look-usability.md,0028-end-to-end-operability.md}`, `CHANGE_LOG.md`.
  - Validation: `backend/.venv/bin/python -m pytest backend/tests` passed (`685 passed, 4 skipped`); `backend/.venv/bin/python -m pytest backend/tests/unit/test_control_operability.py` passed (`8 passed`); backend Ruff checks passed for touched control/CLI/test files; protocol generation checks passed (`scripts/generate_protocol.py --check` and `--check-argparse`); `npm --prefix frontend test --workspaces --if-present` passed (`shared 15`, `cli 48`, `gui 74`); `npm --prefix frontend run build` passed; `git diff --check` passed; functional CLI smoke for `awf control` showed Needs action, Review / close out, Start work, and Top next actions lanes with workflow start commands.
  - Notes: Runtime skips reflect unavailable local STT model artifacts and absent `nvidia-smi` on this host; no new durable authority, scheduler, daemon, approval semantic, or database schema was added.

- Timestamp: 2026-08-30 18:06
  - Host class(es): Linux/WSL2 backend and frontend validation
  - Summary: Implemented the AWF end-to-end operability overhaul. Added backend-derived operator work items, next actions, and run timelines; made `awf control` and `awf status` operator-readable by default; moved AWF-GUI to an Operate home view with prioritized queues, actionable run detail, evidence panels, and registry summaries; and hardened voice runtime behavior so STT is local-only at transcription time and CPU TTS does not depend on broken accelerator providers.
  - Scope: `backend/src/awf/{ops/control.py,ops/run_execution.py,cli/main.py,setup.py,speech/{cli.py,stt_onnx.py,stt_whisper.py,tts_kokoro.py}}`, backend tests under `backend/tests/{unit,runtime,conftest.py}`, frontend shared/CLI/GUI operator surfaces under `frontend/`, `docs/{OperatorsGuide.md,QuickStart-linux.md,QuickStart-windows.md}`, `docs/adr/{0007-cohesive-voice-config-paths.md,0016-voice-model-reconciliation.md,0025-control-center-look-usability.md,0028-end-to-end-operability.md}`, `CHANGE_LOG.md`.
  - Validation: `backend/.venv/bin/python -m pytest backend/tests` passed (`684 passed, 4 skipped`); focused STT/voice checks passed (`9 passed, 3 skipped`); backend Ruff checks passed for touched files; protocol generation checks passed (`scripts/generate_protocol.py --check` and `--check-argparse`); `npm --prefix frontend test --workspaces --if-present` passed (`shared 15`, `cli 48`, `gui 72`); `npm --prefix frontend run build` passed; `git diff --check` passed.
  - Notes: Runtime skips reflect unavailable local STT model artifacts and absent `nvidia-smi` on this host; they are resource boundaries, not acceptance evidence for live STT/GPU execution.
...
- Timestamp: 2026-08-30 07:55
  ...
  - Scope: `backend/src/awf/improvement/{summary.py,diff.py,proposals.py}`, `backend/src/awf/ops/run.py`, `backend/src/awf/cli/main.py`, `backend/tests/unit/{test_improvement_summary.py,test_cli_improvement.py}`, `frontend/shared/src/types.ts`, `frontend/cli/src/commands.ts`, `frontend/gui/src/renderer/{Dashboard.tsx,App.tsx}`, `CHANGE_LOG.md`.
  - Validation: Ruff linting and format checks passed cleanly (`scripts/validate_backend.py lint`); backend unit test suite passed (`336 passed`); backend integration test suite passed (`326 passed`); full frontend monorepo test suite passed (`135 passed` across `@awf/protocol-client`, `awf-cli`, and `awf-gui`); frontend TypeScript compilation and bundle builds passed (`npm run build`).
  - Notes: Provides an operator-first review workflow where workflow runs and self-improvement loops surface ready-to-review changes with single-line summaries, file stats, and unambiguous next CLI/GUI commands.
