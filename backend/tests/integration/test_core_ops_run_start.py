...
from awf.cli.core_ops import CoreOpError, _check_command_args, _repo_python_executable, op_run_resume, op_run_start
...
def test_repo_python_executable_marks_windows_and_linux_venvs(tmp_path):
    ...
    assert _repo_python_executable(tmp_path) == ("linux-venv", str(linux_python))

    windows_python = tmp_path / "backend" / ".venv" / "Scripts" / "python.exe"
    windows_python.parent.mkdir(parents=True)
    windows_python.touch()

    assert _repo_python_executable(tmp_path) == ("windows-venv", str(windows_python))
