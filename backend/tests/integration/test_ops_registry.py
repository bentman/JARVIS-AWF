...
def capability_fixture(fixtures_dir, name: str):
    return fixtures_dir / "guard_registry" / "capabilities" / name / "1.0.0.yaml"


def test_registry_validate_recognizes_capability_and_rejects_garbage(tmp_path, fixtures_dir):
    result = op_registry_validate(capability_fixture(fixtures_dir, "read_file"), kind="capabilities")
...
def test_registry_publish_capability_record_round_trips_and_indexes(tmp_path, fixtures_dir):
    ...
    source = capability_fixture(fixtures_dir, "read_file")
...
def test_registry_retire_then_trust_restores_resolution(tmp_path, fixtures_dir):
    ...
    published = op_registry_publish(
        repo_root, conn, path=capability_fixture(fixtures_dir, "read_file"), kind="capabilities"
    )
