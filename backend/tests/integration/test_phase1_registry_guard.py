...
def load_fixture(fixtures_dir):
    def _load(name: str):
        return load_capability_record(fixtures_dir / "test_phase1" / name)

    return _load


def test_load_r0_capability_record(load_fixture):
    record = load_fixture("test_phase1_read_file_r0.yaml")
...
@pytest.mark.parametrize(
    "fixture, expected",
    [
        ("test_phase1_read_file_r0.yaml", Decision.ALLOW),
        ("test_phase1_write_scratch_file_r1.yaml", Decision.ALLOW),
        ("test_phase1_git_push_r2.yaml", Decision.APPROVAL_REQUIRED),
        ("test_phase1_modify_capability_registry_r3.yaml", Decision.DENY),
    ],
)
...
def test_evaluate_is_pure_and_deterministic(load_fixture):
    record = load_fixture("test_phase1_git_push_r2.yaml")
...
def test_verifier_role_denied_write_capability_above_r0(load_fixture):
    record = load_fixture("test_phase1_write_scratch_file_r1.yaml")  # operation: create, R1
...
def test_verifier_role_allowed_read_only_capability(load_fixture):
    record = load_fixture("test_phase1_read_file_r0.yaml")  # operation: read, R0
    decision, reason = evaluate(record, agent_allowlist=[record.ref], role="verifier")
...
def test_adversary_role_denied_altering_capability(load_fixture):
    record = load_fixture("test_phase1_write_scratch_file_r1.yaml")
