...
def test_control_center_methods_over_jsonrpc(tmp_path, monkeypatch):
    repo_root, conn = make_repo(tmp_path)
    create_run(conn, run_id="run-1", workflow_ref="demo@1.0.0")
    monkeypatch.setattr("awf.ops.control.op_llm_servers", lambda _repo_root, **_kwargs: {"servers": {}})
    monkeypatch.setattr(
        "awf.ops.control.op_llm_serve", lambda _repo_root, _conn, *, action, **_kwargs: {"state": "stopped"}
    )
    monkeypatch.setattr(
...
def test_llm_status_methods_over_jsonrpc(tmp_path, monkeypatch):
    repo_root, conn = make_repo(tmp_path)
    monkeypatch.setattr("awf.ops.llm.op_llm_servers", lambda _repo_root: {"default_server": "llama-server"})
    monkeypatch.setattr("awf.ops.llm.op_llm_models", lambda _repo_root: {"local_models": []})
    monkeypatch.setattr("awf.ops.llm.op_llm_serve", lambda _repo_root, _conn, *, action: {"state": "stopped"})
...
def test_skill_invoke_over_jsonrpc(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("awf.ops.registry.complete", lambda *args, **kwargs: "skill response")
