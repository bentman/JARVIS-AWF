...
def test_governed_machine_activity_without_approval_waits_and_does_not_execute(conn, tmp_path):
    capability_dir = tmp_path / "config" / "app_registry" / "capabilities" / "fs_write"
    capability_dir.mkdir(parents=True)
    (capability_dir / "1.0.0.yaml").write_text(
        "identity: {type: activity, provider: awf, name: fs_write, version: 1.0.0}\n"
        'schema: {input: "", output: ""}\n'
        "effects: {operation: update, reversible: true, idempotent: true, external_side_effect: false}\n"
        "risk_class: R2\n"
        "approval: per-invocation\n"
        "constraints: {filesystem: {allowedRoots: [worktree], maxBytes: 1024}}\n",
        encoding="utf-8",
    )
    create_step(conn, step_id="step-1", run_id="run-1", node_id="write")
    worktree = tmp_path / "worktree"
    worktree.mkdir()
    executor = make_activity_node_executor(repo_root=tmp_path, worktree_path=worktree)
    ...
    output = executor(
        ...
        "step-1",
        {
            ...
            "type": "activity",
            "function": "fs_write",
            "args": {"path": "demo.txt", "content": "must wait"},
        },
    )

    assert output["waiting_input"] is True
    assert not (worktree / "demo.txt").exists()
    row = conn.execute("SELECT status FROM steps WHERE step_id = 'step-1'").fetchone()
    assert row["status"] == "WAITING_APPROVAL"


def test_machine_activity_registry_entries_are_metadata_only():
    for name in MACHINE_ACTIVITY_NAMES:
        registration = ACTIVITY_REGISTRY[name]
        assert registration == ActivityRegistration("machine")
        assert registration.fn is None


def test_governed_machine_activity_without_repo_root_fails_closed(conn):
    create_step(conn, step_id="step-1", run_id="run-1", node_id="read")
    executor = make_activity_node_executor()
    ...
    with pytest.raises(StepFailure):
        executor(
            ...
            "run-1",
            "step-1",
            {"id": "read", "type": "activity", "function": "fs_read", "args": {"path": "README.md"}},
        )
...
def test_governed_machine_activity_registry_entry_cannot_use_plain_activity_path(conn, repo_root, monkeypatch):
    create_step(conn, step_id="step-1", run_id="run-1", node_id="read")
    calls = []

    def plain_activity(_conn, _args):
        calls.append("plain")
        return {"content": "bypassed"}

    executor = make_activity_node_executor(
        activity_registry={"fs_read": plain_activity},
        repo_root=repo_root,
    ...
    assert calls == []
    assert output["path"].endswith("README.md")
