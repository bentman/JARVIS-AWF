...
def voice_manifest(function: str, spec: dict) -> dict:
    return {
        "apiVersion": "awf/v1",
        "kind": "HardwareVoiceManifest",
        "metadata": {"name": function, "version": "1.0.0", "digest": "sha256:test"},
...
def test_parse_url_file():
    manifest = parse_hardware_voice_manifest(
        voice_manifest("wake", {"files": [{"name": "model.onnx", "url": "https://example.com/model.onnx"}]})
    )
...
def test_parse_package_file():
    manifest = parse_hardware_voice_manifest(
        voice_manifest("vad", {"files": [{"name": "silero_vad.onnx", "package": "silero-vad"}]})
    )
...
def test_parse_rejects_file_with_neither_url_nor_package():
    with pytest.raises(HardwareVoiceManifestError):
        parse_hardware_voice_manifest(voice_manifest("wake", {"files": [{"name": "model.onnx"}]}))
...
def test_parse_rejects_unknown_function():
    with pytest.raises(HardwareVoiceManifestError):
        parse_hardware_voice_manifest(voice_manifest("smell", {}))
    ...
    manifest = parse_hardware_voice_manifest(
        {
            "apiVersion": "awf/v1",
            "kind": "HardwareVoiceManifest",
            "metadata": {"name": "stt", "version": "1.0.0", "digest": "sha256:test"},
            "spec": {
                "function": "stt",
                "classes": {
