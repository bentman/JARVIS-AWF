...
| AWF-CLI TUI | Ink 7 (React ≥19.2) + `@inkjs/ui` on Node.js 24 LTS `>=24.15.0`, distributed via npm; inline rendering with append-only transcript (Ink `<Static>` pattern) | https://github.com/vercel/ink |
...
JARVIS/
  ...
  frontend/                                       (one npm-workspaces monorepo — Section 16.2)
    cli/                                          (package awf-cli: TypeScript inline TUI, Node.js 24 LTS >=24.15.0 — Section 16.2)
