...
5. **Download voice models:** `awf-speech models sync`
   ...
   Speech is installed by the hardware-selected setup path. STT uses Faster Whisper where CTranslate2 wheels and CUDA are available, and an ONNX Whisper CPU floor elsewhere; ARM64 QNN is attempted on Windows or Linux when the Qualcomm NPU, QNN provider, and model artifacts are present, then falls back to ONNX CPU STT. Linux OpenWakeWord provisioning installs the usable sibling dependencies normally, installs the package without the unavailable `tflite-runtime` metadata dependency, and verifies the actual `openwakeword`/`onnxruntime` imports.
