...
| Codex CLI | none, for the one-shot `codex exec` subprocess mode this ADR (and ADR-0003) both drive. `skills.config` is a real `config.toml` array, but its entries only enable/disable/select skills Codex has *already* discovered (`{enabled, name?, path?}` - `path` alone is rejected, `enabled` is the only required field) - it does not register a new arbitrary skill folder. The one mechanism that does, `skills/extraRoots/set`, is a `codex app-server --stdio` JSON-RPC method - a persistent session protocol, not a `codex exec` flag or config key. No `config.toml` snippet was found that registers an arbitrary folder for `codex exec`. | live-tested against the installed binary (`codex-cli 0.146.0`): `-c 'skills.config=[{path="...", enabled=true}]'` passes `--strict-config` but the folder never appears in `skills/list`; the same folder appears immediately after `skills/extraRoots/set` over a real `codex app-server --stdio` session - confirming the array/`config.toml` route has no effect and the RPC route works, but is a different protocol entirely |
...
4. For refs explicitly marked `share: true`, additionally copy the skill's full directory (`SKILL.md` + `scripts/`/`references/`/`assets/`, unmodified - no reformatting, the format is already universal) into the worktree at `.claude/skills/<name>/` and `.agents/skills/<name>/`. The `.claude/skills/` path is already read by both Claude Code and GitHub Copilot CLI with no adapter-specific flag; the `.agents/skills/` path is already read by Antigravity, live-tested, given the `--add-dir`/`--new-project` flags this adapter already passes on every invocation - also no adapter-specific flag beyond what's already there. Codex is unsupported for this tier under `codex exec`, live-tested and confirmed, not assumed - registering an arbitrary skill folder is only possible over the `codex app-server --stdio` JSON-RPC protocol (`skills/extraRoots/set`), which is a different, persistent-session invocation model from the one-shot subprocess every adapter in this ADR and ADR-0003 uses. Per ADR-0003's own Scope item 8 standard: not worked around.
...
2. `engine/agent_step.py` - resolve the manifest's `skills` list, apply the trust gate, append surviving `SKILL.md` bodies to the objective, copy `share: true` directories into `.claude/skills/` and `.agents/skills/` for the adapters that read them, write the `events` row. No Codex-specific step - see item 6.
...
6. No Codex renderer for the shared tier - confirmed unsupported under `codex exec`, not left as an open question. If a future revision drives Codex through `app-server --stdio` instead of `exec` (a materially bigger change, out of scope here), `skills/extraRoots/set` is the real mechanism to revisit.
...
- Antigravity's shared tier is settled: `.agents/skills/<name>/SKILL.md` in the worktree, live-verified, no new flag needed beyond what the adapter already passes. Codex's shared tier is also settled, in the other direction: unsupported under `codex exec`, confirmed live rather than assumed - the only real registration mechanism (`skills/extraRoots/set`) belongs to the `app-server --stdio` protocol, a persistent session model no adapter in this ADR or ADR-0003 uses. Of the four adapters, three support the shared tier; Codex does not, and this is not a gap to close later without first deciding whether AWF ever drives Codex through `app-server` at all - a materially bigger question than this ADR's scope.
