...
The explicit-opt-in tier for sharing exists because the underlying mechanism is real across all four adapters: three need no per-invocation work, and Codex needs one additional scratch-directory step. Making it opt-in rather than automatic follows from the authority model: the operator authors the manifest, and the manifest's own explicit marker is the only grant the system can express.

This differs from autonomous, agent-driven skill discovery models that trust the human once at install time. AWF instead treats each use as something the operator's manifest must name. A Skill's `scripts/` are executable code (Section 9.3), so the same authority boundary ADR-0003 applies to MCP tool calls also applies to shared Skill materialization.
...
4. For refs explicitly marked `share: true`, additionally copy the skill's full directory (`SKILL.md` + `scripts/`/`references/`/`assets/`, unmodified) into the worktree at `.claude/skills/<name>/` and `.agents/skills/<name>/`. The `.claude/skills/` path is read by both Claude Code and GitHub Copilot CLI with no adapter-specific flag; the `.agents/skills/` path is read by Antigravity and Copilot CLI given the `--add-dir`/`--new-project` flags Antigravity's adapter already passes on every invocation. For Codex, additionally materialize the same directory under a scratch `$CODEX_HOME`'s `skills/<name>/` (via symlink, avoiding a second real copy on disk) and set `CODEX_HOME` in the adapter's subprocess environment, reusing the scratch-home pattern ADR-0003 established for Antigravity's MCP fix, including copying in the real `auth.json` read-only (session auth lives there and nowhere else) and never touching the operator's real `~/.codex/`.
...
5. `awf.ops.registry` (formerly `cli/core_ops.py`) - `op_registry_publish`/`op_registry_validate` gain a `skills` branch (`op_registry_list` already works generically for this kind, per Context above).
