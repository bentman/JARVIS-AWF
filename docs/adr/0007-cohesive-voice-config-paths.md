...
| `ACCELERATION_CLASSES` and `acceleration_class(profile_id)` in `speech/models.py` (Part B) | not present. STT resolves `cpu`, `cuda`, or `qnn` across ONNX, CTranslate2, and QNN runtimes |
...
| Acceptance: a `-cuda` profile resolves STT to the turbo model; all twelve profile IDs resolve to an `SttRuntime` | an STT readiness device of `cuda` resolves to `deepdml/faster-whisper-large-v3-turbo-ct2` with `device=cuda`, `compute_type=float16`; `qnn` resolves to `whisper-qualcomm-qnn` with `device=qnn`, `compute_type=qnn`; any other device resolves to `onnx-community/whisper-small` with `device=cpu`, `compute_type=int8` |
