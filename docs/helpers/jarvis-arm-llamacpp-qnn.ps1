...
.DESCRIPTION
Manual helper for the JARVIS-AWF Windows ARM64 QNN NPU sidecar build.
The script keeps build trees outside the repo, uses Visual Studio's official
ARM64 developer environment, and stages only the runtime directory in JARVIS-AWF.
...
Path to the JARVIS-AWF repository root.
...
if (-not (Test-Path $jarvisRoot)) {
    throw "JARVIS-AWF root does not exist: $jarvisRoot"
}
...
try {
    Write-Host "JARVIS-AWF root: $jarvisRoot"
    ...
    # Phase 6: create JARVIS-AWF runtime staging directory.
    Invoke-Step "Create JARVIS-AWF runtime staging directory" {
        New-Item -ItemType Directory -Force -Path $jarvisRuntimeDir | Out-Null
        Write-Host "Runtime staging directory: $jarvisRuntimeDir"
    }

    # Phase 7: deploy artifacts into the JARVIS-AWF runtime tree.
    Invoke-Step "Copy llama.cpp QNN runtime artifacts into JARVIS-AWF" {
    ...
    Write-Host ""
    Write-Host "Smoke test before JARVIS-AWF validation:"
    ...
    Write-Host "  - CPU-only output is not sufficient for the windows-arm64-qnn profile."
}
finally {
