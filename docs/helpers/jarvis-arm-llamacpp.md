# JARVIS-AWF ARM llama.cpp Adreno OpenCL Helper
...
The normal workflow runs directly on the ARM64 Snapdragon host. The helper keeps third-party source/build trees outside the repo and stages only the runtime files JARVIS-AWF consumes.
...
```text
runtimes\llama.cpp\windows-arm64-gpu\llama-server.exe
runtimes\llama.cpp\windows-arm64-gpu\OpenCL.dll
```
...
- Stages only the JARVIS-AWF runtime sidecar files.

Third-party work is kept outside the repo by default:

```text
<awfDevRoot>\OpenCL-Headers
<awfDevRoot>\OpenCL-ICD-Loader
...
```text
runtimes\llama.cpp\windows-arm64-gpu\
```
...
- JARVIS-AWF selects the Windows ARM64 Adreno OpenCL llama.cpp profile.
...
- `llm serve start` reaches the local llama.cpp server and reports a running or adopted sidecar.
...
JARVIS-AWF does not select the Adreno profile:
