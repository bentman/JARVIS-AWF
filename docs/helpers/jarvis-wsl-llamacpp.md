# JARVIS-AWF Linux AMD64/WSL2 llama.cpp CUDA Build
...
```text
runtimes/llama.cpp/linux-x64-cuda
```
...
The same pre-model failure pattern has been reported upstream for WSL2 CUDA 13.x builds. The JARVIS-AWF helper therefore fails closed on CUDA 13.x and uses CUDA 12.4 as the qualified WSL build path.
...
From the JARVIS-AWF repository root:
...
- preserves failed stages under `../jarvis-dev/llamacpp-cuda/failed-stage-*`;
...
backend/.venv/bin/python -m awf.cli.main llm servers
backend/.venv/bin/python scripts/validate_backend.py runtime
```

If a GGUF model is staged under `models/llm/<name>/`, a managed local proof is:

```bash
backend/.venv/bin/python -m awf.cli.main llm select llama-server --model <name>
backend/.venv/bin/python -m awf.cli.main llm serve start
backend/.venv/bin/python -m awf.cli.main llm serve status
backend/.venv/bin/python -m awf.cli.main llm serve stop
```
...
- abandoned `runtimes/llama.cpp/linux-x64-cuda.stage.*` directories;
